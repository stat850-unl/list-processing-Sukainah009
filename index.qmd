...
If so, why?
yes, some date are repeated like
...
Sketch goes here![](images/Develop.jpg)
...
1.  Use Tibble to create column list
2.  Map\_\* function to extract
3.  Unnest to data frame
...
series_tibble <- tibble(
  series_name = "drwho_1963",
  episode_data = list(drwho1963)
)

#use map func
episodes_drwho1963 <- series_tibble %>%
  mutate(
    
    episode_ids = map(episode_data, ~map_dbl(.x, "id")),
    episode_names = map(episode_data, ~map_chr(.x, "name")),
    seasons = map(episode_data, ~map_int(.x, "season")),
    episode_nums = map(episode_data, ~map_int(.x, "number")),
    types = map(episode_data, ~map_chr(.x, "type"))
  ) %>%
  select(-episode_data) %>%  # Remove the big list
  unnest(c(episode_ids, episode_names, seasons, episode_nums, types))

# print 5 rows
print(head(episodes_drwho1963, 5))
